...
**Architecture:** `backend/`가 가상 워크스페이스 루트이고 각 BC가 멤버다. `shared_kernel`이 첫 멤버로, 설정·예외·API 스키마·로깅의 교차 관심사를 담는다. 기존 서비스의 `shared_kernel`을 재사용할 수 없으므로(사설 저장소) 필요한 절반만 새로 구현한다.
